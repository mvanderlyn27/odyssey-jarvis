...
const DraggablePostCard = ({ post, isDraggable }: { post: PostWithAssets; isDraggable: boolean }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      <DraggableSchedulerPostCard post={post} isDragging={isDragging} listeners={listeners} isDraggable={isDraggable} />
    </div>
  );
};
                  ...
                  <div key={day.toISOString()} className="flex p-2 space-x-2 border-t">
                    <DroppableSlot
                      ...
                      {morningPosts.map((post) => (
                        <DraggablePostCard key={post.id} post={post} isDraggable={!isMorningPast} />
                      ))}
                    </DroppableSlot>
                    <DroppableSlot
                      id={eveningSlotId}
                      ...
                      {eveningPosts.map((post) => (
                        <DraggablePostCard key={post.id} post={post} isDraggable={!isEveningPast} />
                      ))}
                    </DroppableSlot>
                  </div>
                );
